...
Raised when the data classifier determines that the content sensitivity level (e.g., CONFIDENTIAL, RESTRICTED) is too high for the target model's deployment type (e.g., cloud). Fail fast, do not retry - the user must change the model or redact data.

**Attributes**:

| Attribute | Type | Description |
|-----------|------|-------------|
| `data_level` | `str` | The detected data classification level (e.g., `"CONFIDENTIAL"`) |
