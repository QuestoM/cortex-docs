...
Returns a serializable dict with thresholds and stats.
...
Returns: `trust_scores`, `consistency_scores`, `quarantined` (list of tool names), `total_interactions`.

#### `to_dict() -> Dict[str, Any]`

Returns a serializable dict with trust, consistency, history (last 50 per tool), and total interactions.
...
Returns the current running (incremental) Shapley estimates for all tools.
...
Returns credibility scores for all known tools (union of declared and observed tools).
