...
| `timestamp` | `float` | Unix timestamp (auto-populated via `time.time`). |

---

## Class: CalibrationReport

Generates human-readable calibration reports. JSON-serializable for dashboards. Used internally by `ContinuousCalibrationEngine.report()` and `report_text()`, but can also be used directly.
...
| Method | Signature | Description |
|--------|-----------|-------------|
| `generate` | `() -> Dict[str, Any]` | Generate full JSON calibration report with per-domain ECE, trends, alarms, confidence adjustments, and overall health status (`"healthy"`, `"caution"`, `"warning"`, `"critical"`). |
| `generate_text` | `() -> str` | Generate a human-readable text report. |
...
| `get_alarming_domains` | `() -> List[CalibrationDomain]` | Returns all domains currently in alarm (ECE exceeds threshold). |
