...
| `started_at` | `Optional[float]` | Timestamp when step started executing (default: None) |
| `completed_at` | `Optional[float]` | Timestamp when step completed (default: None) |
...
```python
def can_retry(step: PlanStep) -> bool
```

Check if a step has retries remaining (`retry_count < max_retries`).
...
Retrieve a stored plan by ID. Returns `None` if not found.
