...
### `GoalDNA` (local to drift_engine.py)

**Type**: `@dataclass`

Compact token-set fingerprint for ultra-fast drift detection. This is a **lightweight** `GoalDNA` class defined locally inside `drift_engine.py`. It is **different** from the standalone `corteX.engine.goal_dna.GoalDNA` module, which provides a full-featured implementation with trigram fusion, drift history, and trend analysis. This local version uses simple Jaccard similarity only.
...
| `tokens` | `frozenset` | Goal keywords (words > 2 chars, stop words removed) |
| `trigrams` | `frozenset` | Character trigrams of the goal text |
...
Create a `GoalDNA` fingerprint from a goal string. Extracts meaningful tokens (>2 chars, excluding stop words) and character-level trigrams.
...
Compute Jaccard similarity between goal tokens and action tokens. Returns 0.5 when either token set is empty.
...
- `goal` (str): The goal text (used to create a local `GoalDNA` fingerprint - see `GoalDNA` class above)
...
|----------|------|-------------|
| `goal_dna` | `GoalDNA` | The local `GoalDNA` fingerprint created from the goal text. This is the lightweight `GoalDNA` dataclass defined in `drift_engine.py` (not the standalone `corteX.engine.goal_dna.GoalDNA`). You can call `engine.goal_dna.similarity(text)` directly. |
