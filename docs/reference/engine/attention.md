...
| `timestamp` | `float` | When the change was detected (auto-populated via `time.time`). |
...
| `timestamp` | `float` | When this fingerprint was created (auto-populated via `time.time`). |
...
|-----------|------|---------|-------------|
| `adaptation_filter` | `Optional[Any]` | `None` | Optional `AdaptationFilter` instance for habituation detection (enables SUPPRESSED classification). |
...
| `get_processing_budget` | `(priority: AttentionalPriority) -> ProcessingBudget` | Get recommended budget as a `ProcessingBudget` instance (see below). |
...
Recommended processing parameters for a given attentional priority. Returned by `AttentionalFilter.get_processing_budget()`.

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `max_tokens` | `int` | `4096` | Maximum tokens for the LLM response. |
...
| `use_prediction_engine` | `bool` | `True` | Whether to run the predict-compare-surprise loop. |
| `allow_cache` | `bool` | `False` | Whether to serve from the response cache if available. |
| `context_depth` | `str` | `"full"` | How much context to include: `"full"`, `"summary"`, or `"minimal"`. |
