...
| `allowed_features` | `Dict[str, List[str]]` | `{...}` | Per-plan feature gates. Maps plan name (`"starter"`, `"professional"`, `"enterprise"`, `"unlimited"`) to a list of allowed feature strings. The `"unlimited"` plan uses `["*"]` for all features. |
