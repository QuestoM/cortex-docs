...
- `consent_manager` (`Optional[Any]`): An optional consent manager instance. When attached, the engine auto-resolves `has_consent` from live consent records during `pre_check()`. The consent manager must implement a `check_consent(tenant_id, user_id, purpose) -> bool` method.
...
**Consent auto-resolution**: When a `consent_manager` is attached and the context contains `tenant_id`, `user_id`, and `consent_purpose` keys, the engine automatically resolves `has_consent` from live consent records before evaluating frameworks. Existing explicit `has_consent` values in the context are never overridden. If the consent lookup fails, the engine fails closed (assumes no consent).

**Parameters**:

- `action` (`str`): The action being performed.
- `data_level` (`DataLevel`): Classification level of the data.
- `context` (`Optional[Dict[str, Any]]`): Context dict with framework-specific keys. Additional keys for consent auto-resolution: `tenant_id`, `user_id`, `consent_purpose`.
...
##### `enforce_iso27001` (internal: `_enforce_iso27001`)

ISO 27001 enforcement: information security management, activity logging, and risk assessment checks.

**Context keys**:

| Key | Type | Default | ISO 27001 Section |
|-----|------|---------|-------------------|
...
- **WARN** (optional) if `risk_assessed=False`: risk assessment not performed (6.1.2)
