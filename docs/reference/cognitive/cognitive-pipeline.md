...
**Type**: `Dict[str, float]` (module-level constant)

Default fractional zone budgets used by `CognitiveContextPipeline` when no custom `zone_budgets` are provided. Importable for programmatic access or modification.

```python
from corteX.engine.cognitive.cognitive_context import DEFAULT_ZONE_BUDGETS
